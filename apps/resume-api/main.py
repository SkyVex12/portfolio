...
def notify(message: str):
    # Safe no-op on non-Windows/server environments
...
def generate_person_from_jd(jd_text: str, person: str, avoid_profile: dict | None = None) -> dict:
    """Generate one person's resume JSON (the exact dict used to fill templates)."""
    compress_prompt = build_prompt_compress_jd(jd_text)
    ats_package = call_openai_json(compress_prompt)  # dict
    avoid_profile = avoid_profile or {"banned_starts_first5": [], "verbs_used": []}
    secondary_by_person = build_secondary_by_person(jd_text)
    secondary_json = secondary_by_person.get(person, {})
    ...
    one_json = call_openai_json(person_prompt)
...
def render_resume_text(person: str, pdata: dict) -> str:
    """Simple plain-text renderer for API preview."""
    out = []
    out.append(f"{person} — ATS Resume (preview)")
    out.append("")
    ...
    if skills:
        ...
        for k, vals in skills.items():
            ...
            if isinstance(vals, list):
                out.append(f"- {k}: {', '.join(str(v) for v in vals)}")
            else:
                out.append(f"- {k}: {vals}")
        out.append("")
    ...
    if exp:
        ...
        for i in range(1, COMPANY_COUNT + 1):
            bullets = exp.get(f"company_{i}", []) or []
            if not bullets:
                continue
            out.append(f"Role {i}")
            for b in bullets:
